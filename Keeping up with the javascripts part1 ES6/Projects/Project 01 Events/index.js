...
accountdetails.addEventListener("click", function(event) {
  ...
  document.getElementById(
    "accountDetailsEmail"
  ).value = document.getElementById("emailDashboard").value;
  ...
  document.getElementById("accountDetailsFirstName").style.backgroundColor =
    "lightblue";
  ...
  document.getElementById("accountDetailsLastName").style.backgroundColor =
    "lightblue";

  document.getElementById("accountDetailsEmail").readOnly = true;
  document.getElementById("accountDetailsEmail").style.backgroundColor =
    "lightblue";

  document.getElementById("accountDetailsPassword").value = "HASHED";
  document.getElementById("accountDetailsPassword").readOnly = true;
  document.getElementById("accountDetailsPassword").style.backgroundColor =
    "lightblue";
});
...
const EmailValidateUpdate = (field, alerttxt) => {
  with (field) {
    ...
    else if (document.getElementById("newEmail").value in localStorage) {
      const element = document.getElementById("newEmail");
      const text =
        "A user account already exists with this email address.  Please try a different email address.";
...
const FirstNameValidateUpdate = () => {
  ...
  if (newFirstName.length >= 1) {
    if (
      ...
      const text =
        "The new first name is already the currently specified first name";
...
const LastNameValidateUpdate = () => {
  ...
  if (newLastName.length >= 1) {
    if (
      ...
      const text =
        "The new last name is already the currently specified last name";
...
const updateLoginFromAccountDetailsUpdate = () => {
  ...
  document.getElementById("fullNameDashboard").value =
    usr1.firstName + " " + usr1.lastName;
...
const dashboardLoggedInButton = document.getElementById(
  "dashboardLoggedInButton"
);
...
const accountDetailsUpdateButton = document.getElementById(
  ...
  if (document.getElementById("emailLogin").value in localStorage) {
    ...
    hash("SHA-256", uipwd).then(hashed => {
      ...
      if (pword === hashedpwd) {
        ...
        document.getElementById("todoListTitle").style.backgroundColor =
          "white";
        ...
        document
          .getElementById("accountDetailsButton")
    ...
    const text =
      "This User does not exist please use an existing account or sign up";
...
dashboard.addEventListener("click", function(event) {
    ...
    if (key === document.getElementById("todoListTitle").value) {
      value.forEach(function(item) {
        ...
        if (itemValue === newListItem.value.trim()) {
          ...
          const text =
            "List item with the same name has already been added to list";

          MessageAlert(element, text, "todoListItem");
        }
      });
    }
  });
  ...
  if (newListItem.value !== "" && itemExists === false) {
    let htmlString1 =
      '<input type="checkbox" class="listItems" ><label class="listItems">';
    let htmlString2 = "</label>";

    usr[todoList.toUpperCase()].push(
      htmlString1 + newListItem.value + htmlString2
    );
...
todoListTitleFocus.addEventListener(
  "focus",
  event => {
    // event.preventDefault();
...
todoListItemFocus.addEventListener(
  "focus",
  event => {
    // event.preventDefault();
...
passwordFocus.addEventListener(
  "focus",
  event => {
    // event.preventDefault();
...
const dashboardAddItemButton = document.getElementById(
  "dashboardAddItemButton"
);
...
var excludeblur = "";
const dashboardCancelNewListNameButtonMO = document.getElementById(
  "dashboardCancelNewListNameButton"
);
...
const createNewListButton = document.getElementById(
  "dashboardCreateListButton"
);
createNewListButton.addEventListener("click", event => {
  ...
  document
    .getElementById("dashboardEditListNameButton")
    .classList.add("hidden");
  document
    .getElementById("dashboardSaveNewListNameButton")
    .classList.add("hidden");
});
...
changeListItemChkBox.addEventListener("change", event => {
  ...
  const listItem = event.target.closest("#todoListContentItems input")
    .parentNode;
...
const dashboardEditListNameButton = document.getElementById(
  "dashboardEditListNameButton"
);
dashboardEditListNameButton.addEventListener(
  "click",
  event => {
    event.preventDefault();
    ...
    document.getElementById("todoListTitle2").style.backgroundColor =
      "lightblue";
    document.getElementById("todoListTitle2").value = document
    ...
    document
      .getElementById("todoListTitle")
      .setAttribute("placeholder", "INPUT NEW LIST NAME");
    document
      .getElementById("dashboardSaveNewListNameButton")
      .classList.remove("hidden");
    document
      ...
      .classList.add("hidden");
    document
      .getElementById("dashboardCancelNewListNameButton")
      .classList.remove("hidden");
...
const dashboardCancelNewListNameButton = document.getElementById(
  "dashboardCancelNewListNameButton"
);
dashboardCancelNewListNameButton.addEventListener(
  "click",
  event => {
    event.preventDefault();
    ...
    document.getElementById("todoListTitle2").classList.add("hidden");
    document.getElementById("todoListTitle2").style.backgroundColor = "white";
    document.getElementById("todoListTitle").value = document.getElementById(
      "todoListTitle2"
    ).value;
    document.getElementById("todoListTitle2").value = "";
    ...
    document
      .getElementById("todoListTitle")
      .setAttribute("placeholder", "INPUT LIST NAME");
...
const dashboardSaveNewListNameButton = document.getElementById(
  "dashboardSaveNewListNameButton"
);
dashboardSaveNewListNameButton.addEventListener("click", function(event) {
  ...
  const oldListName = document
    .getElementById("todoListTitle2")
    .value.toUpperCase();
...
const signupWithDetailsButton = document.getElementById(
  "signupWithDetailsButton"
);
signupWithDetailsButton.addEventListener("click", function(event) {
  ...
  document.getElementById("emailDashboard").value = document.getElementById(
    "emailSignUp"
  ).value;
...
clearSignupFieldOnFocus.addEventListener(
  "click",
  function(event) {
    // event.preventDefault();
...
accountDetailsFieldOnFocus.addEventListener(
  "click",
  function(event) {
    event.preventDefault();
    //clears message alert box
